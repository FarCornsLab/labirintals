...
От: `client`.  
Доступно для: игрока и наблюдателя.  
Параметры:  
* `name` (string) - (обязательно только для игрока) необходимо указать имя присоединяемого клиента, если он является игроком;
* `token` (string) - (обязательно только для наблюдателя) необходимо указать ключ сервера, если пользователь хочет просматривать всё поле во время игры и не является игроком.
...
От: `server`.  
Параметры:  
* `player`(Player) - (необязательное поле, возвращается если клияент является игроком) содержит информауию об игроке. 
...
#### `get_map`:
Запрашивает у сервера карту игры.  
От: `client`.  
Доступно для: наблюдателя.  
Ответ: `map`. 
...
(Пример возвращаемой карты можно увидеть в команде `game_result`).
